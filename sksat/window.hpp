...
class window_base {
public:
	window_base() : xsize(default_xsize), ysize(default_ysize) {}

	virtual void open(size_t x, size_t y) = 0;
	void open(sksat::string &t){ set_title(t); open(xsize, ysize); }
	void open(sksat::string &t, size_t x, size_t y){ set_title(t); open(x,y); }
	void open(){ open(xsize, ysize); }
	...
	virtual void set_size(size_t x, size_t y){ xsize=x; ysize=y; update_size(x,y); }
	void set_xsize(size_t x){ set_size(x, ysize); }
	void set_ysize(size_t y){ set_size(xsize, y); }
	size_t get_xsize(){ return xsize; }
	size_t get_ysize(){ return ysize; }
protected:
